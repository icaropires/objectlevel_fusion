...
TEST(TestTemporalAlignment, convertToObjectModel) {
    const ctra_array_t state = {4.0, -2.0, M_PI/2, 5.0, M_PI/7, 4.0};
    const float yaw = M_PI/2, velocity_heading = M_PI/2, acceleration_heading = M_PI/2;

    state_t result = TemporalAlignerEKF::format_to_object_model(state, yaw, velocity_heading, acceleration_heading);
...
TEST(TestTemporalAlignment, convertFromObjectModel) {
    const state_t state = {4, -2, 0, 5, 0, 4, M_PI/2, M_PI/7};
    const float heading = M_PI/2;

    ctra_array_t result = TemporalAlignerEKF::format_from_object_model(state);

    const float precision = 1e-4;
    ctra_array_t expected = {4.0, -2.0, heading, 5.0, M_PI/7, 4.0};
    ASSERT_THAT(
        result,
        testing::Pointwise(testing::FloatNear(precision), expected)
    );
